...
function ChartSkeleton() {
    return (
        <div className="h-full w-full flex flex-col items-center justify-center gap-3 bg-[#0d0d0f]">
            <div className="relative h-8 w-8">
                <div className="absolute inset-0 animate-spin rounded-full border-2 border-white/[0.05] border-t-indigo-500/60" />
            </div>
            <p className="text-[11px] text-zinc-600 tracking-widest uppercase">Loading chart…</p>
        </div>
    );
}

/* ── Full TradingView advanced chart — dynamically imported, NO SSR ── */
// Wrapped in next/dynamic with ssr:false to:
//   1. Prevent hydration mismatches (TradingView mutates DOM via scripts)
//   2. Avoid blocking the main thread during server render
//   3. Allow the page shell + AI analysis to appear instantly
...
const AdvancedChart = dynamic<AdvancedChartProps>(
    () => import("./advanced-chart").then((m) => m.AdvancedChart as React.ComponentType<AdvancedChartProps>),
    {
        ssr: false,
                    ...
                    {!isLoading && instrument && (
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_360px]">
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                {/* TradingView Chart — loaded client-side only via next/dynamic */}
                                <div className="h-[400px] sm:h-[520px] overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0d0d0f]">
                                    <AdvancedChart
                                        symbol={symbol}
                                        widgetSymbol={TV_WIDGET_SYMBOLS[symbol] ?? `FX:${symbol}`}
                                        bullish={bullish}
                                    />
                                </div>
